...
- managed tables have a prototype local sidecar index path for `PRIMARY KEY`, `UNIQUE`, `CREATE INDEX`, `ALTER INDEX RENAME TO`, and `DROP INDEX`; simple equality filters on single-column indexes can use the sidecar instead of scanning every managed Parquet file
