...
- current CLI SQL coverage now exercises the prototype managed-table index path for `PRIMARY KEY`, column/table `UNIQUE`, `CREATE INDEX`, `ALTER INDEX RENAME TO`, `DROP INDEX`, `REINDEX INDEX`, and `REINDEX TABLE`, including unique-key rejection, duplicate-name rejection, constraint-backed-index protection, atomic failure rollback for `CREATE UNIQUE INDEX` and `ALTER TABLE ... ADD PRIMARY KEY`, versioned index-manifest publication, indexed equality/`IN`/bounded-range lookup, sidecar rebuild recovery, and post-`TRUNCATE` index recovery through the CLI
- current CLI SQL coverage now includes PostgreSQL wire and Flight SQL listener validation for the current standalone-index lifecycle and predicate slice (`CREATE INDEX`, `ALTER INDEX RENAME TO`, `DROP INDEX`, `REINDEX`, `IN`, and bounded-range lookup) plus protocol-visible rejection of dropping a primary-key backing index
- current engine and CLI SQL coverage now includes the prototype query log: an engine test proves logged non-streaming queries are readable through `system.query_log`, and a CLI-driven PostgreSQL-wire test submits SQL through a live listener and reads the resulting `system.query_log` row back through the CLI
